    ...
    > MerkleTree<E, A, S, BaseTreeArity, SubTreeArity, TopTreeArity>
{
    ...
    fn read_top_tree_leafs<Arity: Unsigned>(
        ...
        for i in challenges {
            ...
            // Get the leaf index within the sub-tree.
            let leaf_index = i % tree_leafs;
            let mut node = tree.read_leafs_with_fill_buf(
                vec![leaf_index],
                rows_to_discard,
                fill_buf,
    ...
    fn leaf_index_to_segment_range(
        &self,
        tree_index_getter: &dyn Fn(usize) -> Result<usize>,
        leaf_index_getter: &dyn Fn(usize) -> Result<usize>,
        i: usize,
    ) -> Result<(usize, SegmentRange)> {
        match tree_index_getter(i) {
            Ok(tree_index) => match leaf_index_getter(i) {
                Ok(leaf_index) => {
                    Ok((tree_index, SegmentRange{
                        segment_width: 0,
                        range: Range {
                            index: leaf_index,
                            start: leaf_index,
    ...
    fn get_base_tree_ranges(
        ...
        let tree_index_getter = |j| -> Result<usize> {
            self.get_base_tree_index(j)
        };
        let leaf_index_getter = |j| -> Result<usize> {
            self.get_base_tree_leaf_index(j)
        };
    ...
    fn read_tree_ranges(&self, tree_ranges: Vec<TreeRanges>) -> Vec<Vec<u8>> {
        ...
        for tree_range in tree_ranges {
            ...
            match self.read_ranges_into(tree_range, &mut buf) {
                Ok(results) => {
    ...
    pub fn read_leafs(
        ...
        let leafs_data = self.read_leafs_with_fill_buf(challenges.clone(), rows_to_discard, false, None)?;
        let tree_leafs_data = self.read_tree_leafs_data(leafs_data)?;
        self.read_leafs_with_fill_buf(challenges.clone(), rows_to_discard, true, Some(&tree_leafs_data))
    }
    ...
    fn read_tree_leafs_data(
        ...
        for data in leafs_data {
            base_ranges.push(data.range);
            total_buf_size += data.range.buf_end - data.range.buf_start;
        ...
        let results = store.read_ranges_into(base_ranges.clone(), &mut data_copy)?;

        let mut base_data = Vec::new();
        for (i, result) in results.iter().enumerate() {
            match result {
                Ok(_) => base_data.push(data_copy[base_ranges[i].buf_start..base_ranges[i].buf_end].to_vec()),
                Err(_) => base_data.push(Vec::new()),
            }
        }
                ...
                for (i, range) in ranges.iter().enumerate() {
                    nodes.push(
                        LeafNodeData {
                            ...
                            */
                            data: if fill_buf {
                                self.read_base_leaf_data_from_tree_leafs_data(range.index, tree_leafs_data.unwrap())
                            } else {
                                Err(anyhow!("fail to read challenge {}", range.index))
                            },
    ...
    fn read_base_leaf_data_from_tree_leafs_data(
        &self,
        i: usize,
        tree_leafs_data: &TreeLeafData,
    ) -> Result<Vec<u8>> {
        for (idx, range) in tree_leafs_data.base_ranges.iter().enumerate() {
            if range.index == i {
                return Ok(tree_leafs_data.base_data[idx].clone());
            }
        }
        Err(anyhow!("fail to find challenge {}", i))
    }
    ...
    /// Returns merkle leaf at index i
    #[inline]
    pub fn read_ranges_into(&self, tree_range: TreeRanges, buf: &mut [u8]) -> Result<Vec<Result<usize>>> {
        match &self.data {
            Data::TopTree(sub_trees) => {
                // Locate the top-layer tree the sub-tree leaf is contained in.
                ...
                tree.read_ranges_into(tree_range, buf)
            }
            Data::SubTree(base_trees) => {
                // Locate the sub-tree layer tree the base leaf is contained in.
            ...
            Data::BaseTree(data) => {
                ...
                data.read_ranges_into(ranges, buf)
            }
        }
    }
